...
class UserMessagePostView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(data={"message": "Successfully delivered message."}, status=status.HTTP_201_CREATED)
    ...
    def get(self, request, *args, **kwargs):
        ...
        response = model.generate_content(
            UserDialog.objects.filter(
                user=serializer.validated_data["user"], chat_room=serializer.validated_data["chat_room"]
            )
            .first()
            .text
        )
